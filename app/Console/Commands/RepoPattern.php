...
/**
 * Générateur de pattern complet pour Laravel 13.
 *
 * Conventions Laravel 13 appliquées :
 * - Injection de dépendances via le constructeur
 * - $guarded = [] sur les modèles (recommandé L11+)
 * - Routes dans routes/api.php avec ->names() automatique
 * - Retour de code exit (Command::SUCCESS / Command::FAILURE)
 * - Support du Repository pattern (interface + implémentation + binding)
 * - Rollback automatique en cas d'échec partiel
 *
 * Usage :
 *   php artisan make:pattern Invoice
 *   php artisan make:pattern Invoice --full
 *   php artisan make:pattern Invoice --full --repository
 *   php artisan make:pattern Invoice --all --soft-delete --force
 *   php artisan make:pattern Invoice --dry-run
 */
#[Signature('app:repo-pattern
...
#[Description('Génère le pattern API complet (Model, Repository, Service, Controller, Requests, Resource, Policy, Tests, Routes)')]
class RepoPattern extends Command
{
    ...
    private function runDryRun(string $name): int
    {
        ...
        $this->components->twoColumnDetail(
            ...
            'php artisan make:pattern '.$name.' --full'
        );
    ...
    /**
     * Injecte HasUuids + HasOrganization dans le modèle généré par Laravel.
     */
    ...
    private function ensureRepositoryProviderBinding(string $name): void
    {
        ...
        // Insère le binding dans le provider existant
        ...
        // Injection du binding dans register()
    ...
    /**
     * Ajoute la route apiResource dans routes/api.php.
     * Si un groupe auth:sanctum existe, insère à l'intérieur.
     */
...
/**
 * Gère les requêtes API pour la ressource {$name}.
 *
 * Routes :
 *   GET    /api/{$plural}          → index
 *   POST   /api/{$plural}          → store
 *   GET    /api/{$plural}/{$var}   → show
 *   PUT    /api/{$plural}/{$var}   → update
 *   DELETE /api/{$plural}/{$var}   → destroy
 */
...
{
    /**
     * Transforme le modèle en tableau JSON.
     *
...
/**
 * Trait ApiResponse — Centralise toutes les réponses JSON de l'API.
 */
trait ApiResponse
{
    // --- Succès ---------------------------------------------------------------
    ...
    /**
     * Exécute un callback et capture automatiquement les exceptions courantes.
     */
